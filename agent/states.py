...
class Implementation(BaseModel):
    filepath: str = Field(
        description="The path to the file that is currently being created or modified"
    )
    content: str = Field(description="content of the file.")


class CoderState(BaseModel):
    task_plan: ImplementationPlan = Field(
        description="The current state of the implementation plan, including which tasks have been completed and which are still pending."
    )
    current_step_index: int = Field(
        0, description="The index of the current implementation step being worked on."
    )
